...
always_comb begin
    if ((((core_instr_req_i.req & core_instr_req_ff.req == 1'b0)                    |
        (core_instr_req_i.req & core_instr_resp_gnt_i)) & core_instr_req_ff.we == 1'b0)|
        (core_instr_req_i.req & core_instr_req_ff.we & core_instr_resp_rvalid_i))// Accept req while N req not 0)
      load = 1'b1;
    else
      load = 1'b0;
end
